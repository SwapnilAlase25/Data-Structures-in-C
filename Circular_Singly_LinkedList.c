...
else if(*TempHead==NULL)					//ADD AT POS AT BEG
{
		*TempHead=Temp;
		Temp->next=*TempHead;
...
t1=*TempHead;
	...
	if(pos==count)				// ADD AT POS AT END
	{
		Temp->next=*TempHead;
		t1->next=Temp;
	}
	else					// ADD AT POS IN MIDDLE
	{
		Temp->next=t1;
		t2->next=Temp;
	}
